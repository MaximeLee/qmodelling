        ...
        for j in range(n):
            for k in range(n):
                for l in range(n):
                    ...
                    #G[i,j] += P[k,l]*(J-0.5*K)
                    Vcoulomb[i,j] += density*J
                    Vx[i,j]       += density*(-0.5*K)
    return Vcoulomb, Vx
...
def scf_loop(molec,Nmax=200,eps=1e-8):
    ...
    for _ in range(Nmax):
        ...
        P = P_mat(c,n)
        Ecore, Ecoulomb, Ex = compute_Eelec(P,Hcore,Vcoulomb,Vx,n)
