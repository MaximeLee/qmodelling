#!/usr/bin/env python
"""DFT LDA RKS code with Becke partionning for H2"""
import numpy as np
...
def Vep_int(orbitals, molecule):
    """electron proton operator"""
    n = len(orbitals)
    Vep = np.zeros([n, n])
            ...
            for j in range(n):
                ...
                for k in range(ni):
                    ...
                    for l in range(nj):
                        ...
                        Vep_kl = (
                            -Z * ckl * PrimitiveGaussian.electron_proton_int(bk, bl, Xp)
                        )
                        ...
                        for jj in range(nj):
                            ...
                            for kk in range(nk):
                                ...
                                for ll in range(nl):
                                    ...
                                    Vee[
                                        i, j, k, l
                                    ] += cijkl * PrimitiveGaussian.electron_electron_int(
                                        bii, bjj, bkk, bll
                                    )
        ...
        for j in range(n):
            ...
            for ii in range(n):
                for jj in range(ii + 1, n):
                    ...
                    # Chebyshev + Lebedenev quadrature
                    # + variable substitution for radial component (R/Mu)
                    Vxc[i, j] += (
                        4.0
                        * pi
                        * np.einsum(
...
def exc(rho):
    ...
    # Dirac exchange
    ex = -(3.0 / 4.0) * (3.0 / pi * rho) ** (1.0 / 3.0)
    #ex = -(3.0 / 4.0) * (3.0 / pi * rho) ** (4.0 / 3.0)
    # ex *= 9.0/8.0
    ...
    for ii in range(n):
        for jj in range(ii + 1, n):
            ...
            Exc += (
                ...
                * pi
                * np.einsum(
                    "ij,ij", wcell * rho[ii, jj, 0] * exc(rho[ii, jj, 0]) * subs, Wint
                )
            )
            ...
            Exc += (
                ...
                * pi
                * np.einsum(
                    "ij,ij", wcell * rho[ii, jj, 1] * exc(rho[ii, jj, 1]) * subs, Wint
                )
            )
...
sin_i = np.sin(ir * pi / (Nr + 1))
cos_i = np.cos(ir * pi / (Nr + 1))

# Chebyshev quadrature points on [-1,1]
Mu_quadrature = (Nr + 1.0 - 2.0 * ir) / (Nr + 1.0) + 2.0 / pi * (
    1.0 + 2.0 / 3.0 * sin_i**2
) * cos_i * sin_i
...
alpha1 = 0.3425250914e01
alpha2 = 0.6239137298e00
alpha3 = 0.1688554040e00
# coefficients of linear combination
c1 = 0.1543289673e00
c2 = 0.5353281423e00
c3 = 0.4446345422e00
# coeff = (c1,c2,c3)
STO3G_coeff = np.array([c1, c2, c3])
...
for niter, d in enumerate(dlin):
    ...
    orbitalH1 = Orbital(STO3G_coeff, basisH1)
    orbitalH1.x = x1
    ...
    orbitalH2 = Orbital(STO3G_coeff, basisH2)
    orbitalH2.x = x2

    orbitalsHH = (orbitalH1, orbitalH2)

    # scf loop
    Ecore_final, Ecoulomb_final, Exc_final, c_final, _ = scf_loop(
        orbitalsHH, H_molecule, params
    )
    Eelec_final = Ecore_final + Ecoulomb_final + Exc_final

    # proton-proton interaction
    Epp_final = Vpp_int(H_molecule)

    E_list.append(Epp_final + Eelec_final)
    Ecore_list.append(Ecore_final)
    Ecoulomb_list.append(Ecoulomb_final)
    Exc_list.append(Exc_final)
    Epp_list.append(Epp_final)

    print(f"Opti {niter+1}/{nd} done!")
    print(f"distance : {d}")
    print(f"Coefficients : {c_final[:,0]}\n")
...
np.savetxt('rks_lda_h2.txt',np.stack([dlin,E_list],axis=1))
